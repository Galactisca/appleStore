...
const productObj = {"MA29":1, "MP1666":2, "MP1340":3, "IPP35":4, "IP13":5, "IPM20":6, "IPB10":7, "IP13PM":8, "IP13P":9, "IP12PM":10};
...
const loadHistory = async () => {
    ...
    const rawProductInfo = await fetch('https://61814ec932c9e20017804764.mockapi.io/products?');
    const productInfo = await rawProductInfo.json();

        console.log(history.length);
    for (let i=0; i<history.length; i++){
        loadTransaction(history[i].date, history[i].productId, history[i].money, productInfo);
    }
}
...
function loadTransaction(date, products, money, productsInfo){
    ...
    dateMoney.innerHTML = `
        ...
        <div class="history-money"> ${formatMoney(money)} </div>
    `;
    ...
    for (let i=0; i<productIds.length; i++){
        ...
        item.innerHTML = `
            <div class="history-img" style="background-image: url('${productInfo.src}');"></div>
            <div class="history-name">${productInfo.name}</div>
            <div class="history-quantity">${products[productIds[i]]}</div>
            <div class="history-price">${formatMoney(productInfo.price)}</div>
        `;
